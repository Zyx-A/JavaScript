...
//---------------------------------------------------------------------------------------------------------
let ckStr = ($.isNode() ? process.env.cdlm_data : $.getdata('cdlm_data')) || '';
let msg, ck;
let ck_status = true;
...
//---------------------------------------------------------------------------------------------------------
let VersionCheck = "3435"
let Change = '查询 !'
let thank = `\n感谢 xx 的投稿\n`
//---------------------------------------------------------------------------------------------------------

async function tips(ckArr) {
	let Version_latest = await Version_Check('cdlm');
...
!(async () => {
	let ckArr = await Variable_Check(ckStr, "cdlm_data");
	await tips(ckArr);
...
async function userInfo() {
	let Option = {
		url: `${hostname}/user/info?token=${ck}`,
		headers: cdlm_headers,
	};
...
async function task_list() {
	let Option = {
		url: `${hostname}/task/day?token=${ck}`,
		headers: cdlm_headers,
	};
	let result = await httpGet(Option, `任务列表`);

	if (result.code == 0) {
		task_Arr = result.data.baseData;
		if (task_Arr.sign.todayCount < task_Arr.sign.mustCount) {
			DoubleLog(`签到:这个月还可以签到,去签到了鸭!`);
			await signin();
		} else {
			DoubleLog(`签到:这个月不能签到了!`);
		}
		if (task_Arr.share.todayCount < 2) {
			DoubleLog(`转发:${task_Arr.share.todayCount}/2`);
			DoubleLog(`开始 转发`);
			await video_list();
			await share();
			...
			DoubleLog(`转发:今天已经 转发 过了!`);
		}
		if (task_Arr.comment.todayCount < 2) {
			DoubleLog(`评论:${task_Arr.comment.todayCount}/2`);
			DoubleLog(`开始 评论`);
			await video_list();
			await add_comment();
			...
			DoubleLog(`评论:今天已经 评论 过了!`);
		}
		if (task_Arr.video.todayCount < 2) {
			DoubleLog(`看视频:${task_Arr.video.todayCount}/2`);
			DoubleLog(`开始 看视频`);
			await video_list();
			await Watch_video();
			...
			DoubleLog(`看视频:今天已经 看视频 过了!`);
		}
	} else if (result.code == 101) {
		DoubleLog(`任务列表:${result.msg} `)
	} else {
...
async function signin() {
	let Option = {
		url: `${hostname}/action/action?token=${ck}&type=7`,
		headers: cdlm_headers,
	};
	let result = await httpGet(Option, `签到`);
 ...
 * 视频列表   get
 */
async function video_list() {
	let Option = {
		url: `${hostname}/content/list?token=${ck}&keyword=&page=1&pageSize=10&source=3&orderBy=1&categoryId=2`,
		headers: cdlm_headers,
	};
	let result = await httpGet(Option, `视频列表`);

	if (result.code == 0) {
		DoubleLog(`视频列表:${result.msg}`);
...
async function share() {
	let Option = {
		url: `${hostname}/action/action?token=${ck}&rid=${rid}&type=5&source=3&contentType=2`,
		headers: cdlm_headers,
	};
	let result = await httpGet(Option, `转发`);

	if (result.code == 0) {
		DoubleLog(`转发:${result.msg}`);
		await wait(5);
	} else {
		DoubleLog(`转发: 失败 ❌ 了呢,原因未知!`);
		console.log(result);
	}
}
...
async function add_comment() {
	let Option = {
		url: `${hostname}/comment/comment?token=${ck}&content=${text}&bodyType=3&bodyId=${rid}`,
		headers: cdlm_headers,
	};
	let result = await httpGet(Option, `评论`);

	if (result.code == 0) {
		DoubleLog(`评论:${result.msg}`);
		await wait(5);
	} else {
...
async function Watch_video() {
	let Option = {
		url: `${hostname}/action/action?token=${ck}&rId=${rid}&type=8&source=3&contentType=2`,
		headers: cdlm_headers,
	};
	let result = await httpGet(Option, `看视频`);

	if (result.code == 0) {
		DoubleLog(`看视频:${result.msg}`);
		await wait(5);
	} else {
		DoubleLog(`看视频: 失败 ❌ 了呢,原因未知!`);
		console.log(result);
	}
}
